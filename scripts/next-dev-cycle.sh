...
if [ -z "$NEXT_VERSION" ]; then
    NEXT_VERSION=$(sed -nE 's/.*([0-9]+\.[0-9]+).*/\1/p' pkg/horreum/version.txt)
fi
...
# Extract MAJOR (everything before the first dot)
MAJOR=${NEXT_VERSION%%.*}
# Extract MINOR (everything after the first dot)
MINOR=${NEXT_VERSION##*.}
# Increment the MINOR version
NEW_MINOR=$((MINOR + 1))

# Update to the next patch to remove the .dev
./scripts/update-version.sh -n "$NEXT_VERSION" >/dev/null 2>&1
# And then do minor release with dev
NEXT_RELEASE_VERSION="$MAJOR.$NEW_MINOR"
NEXT_DEV_VERSION=$(./scripts/update-version.sh -n "$NEXT_RELEASE_VERSION" -d | tail -n1)
